...
class NumpyinMnist():
    ...
    def train(self, training_data,test_data, epoches, batch_size, lr):
        ...
        for j in range(epoches):
            ...
            for batch_single in batches:
                 loss=self.update_mini_batch(batch_single, lr)
            if test_data:
    ...
    def update_mini_batch(self, batch, lr):
        ...
        for x, y in batch:
            ...
            batch_gradient_w = [accu + cur for accu, cur in zip(batch_gradient_w, gradient_w)]
            batch_gradient_b = [accu + cur for accu, cur in zip(batch_gradient_b, gradient_b)]
            batch_loss+=loss
        batch_gradient_w = [w / len(batch) for w in batch_gradient_w]
        batch_gradient_b = [b / len(batch) for b in batch_gradient_b]
        loss=loss/len(batch)
        # w = w - lr * nabla_w
        self.weight = [w - lr * nabla for w, nabla in zip(self.weight, batch_gradient_w)]
        self.bias = [b - lr * nabla for b, nabla in zip(self.bias, batch_gradient_b)]
