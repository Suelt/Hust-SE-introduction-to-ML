...
class NumpyinMnist():
    ...
    def backpropagation(self, x, y):
        ...
        delta_1 = intermediate_list[-1] * (1 - intermediate_list[-1]) * (intermediate_list[-1] - y)
        gradient_b[-1] = delta_1
        intermediate_output=intermediate_list[-2].T
        delta_2=delta_1@intermediate_output
        gradient_w[-1] = delta_2
        ...
        delta_1 = np.dot(self.weight[-1].T, delta) * a * (1 - a)

        gradient_b[-2] = delta_1
        delta_2=delta_1@x.T
        gradient_w[-2] = delta_2
...
if __name__ == '__main__':
    ...
    demo=NumpyinMnist()
    demo.train(train_data,10,10,0.1,test_data)
