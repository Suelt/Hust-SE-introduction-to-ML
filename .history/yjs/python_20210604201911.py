...
class node:
    def __init__(self, layer, next_layer=None):
        ...
        self.value = np.zeros((self.layer, 1)) # 当前节点的值，如 X1，y1
        if self.next_layer != None:
            self.w = np.random.randn(self.next_layer, self.layer) # 当前节点到下一节点的权值 randn 为标准差为 1 的正态分布
...
class data:
    ...
    def loadData(self, mnist_url):
        ...
        training_data = zip(training_inputs, training_results)# validation_inputs = [np.reshape(x, (784, 1)) for x in validation_data[0]]
...
class net:
 ...
 # self.printResult(j)
                if test_data:
                    ...
                    print("Epoch {} : {} / {}".format(j, self.evaluate(test_data), n_test), end="")
                    # j 为迭代期序号
                    # evaluate(test_data)为测试通过的数据个数
                    # n_test 为测试数据集的大小
                else:
                    print("Epoch {} complete".format(j), end="")
                self.printResult(j)
...
def main():
   # m_net = net()
    ...
    (training_inputs, training_results), (x_test, y_test) = mnist.load_data()
    print(x_test.shape,y_test.shape)
    result=pd.merge(pd.dataframe(x_test),pd.dataframe(y_test))
    #test_data=
    #test_data = m_data.loadData('D:/mnist.pkl.gz')
   # m_net.train(training_inputs, training_results, 50000, test_data)
    #print("Error: %.2f%%" % ((sum(score)/len(score))*100))
