...
class NumpyinMnist():
    ...
    def update_mini_batch(self, batch, lr):
        ...
        for x, y in batch:
            ...
            # [w1,w2,w3]
            gradient_w_, gradient_b_,loss_ = self.backpropagation(x, y)
            gradient_w = [accu + cur for accu, cur in zip(gradient_w, gradient_w_)]
            gradient_b = [accu + cur for accu, cur in zip(gradient_b, gradient_b_)]
            loss+=loss_
        gradient_w = [w / len(batch) for w in gradient_w]
        gradient_b = [b / len(batch) for b in gradient_b]
        loss=loss/len(batch)
        # w = w - lr * nabla_w
        self.weight = [w - lr * gradient for w, gradient in zip(self.weight, gradient_w)]
        self.bias = [b - lr * gradient for b, gradient in zip(self.bias, gradient_b)]
