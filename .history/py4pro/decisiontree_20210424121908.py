...
print('min_sample_leafs:%d'%num_i)

print('max_leaf_nodes:%d'%num_j)

print('accuracy_score:%f'%max)
...
ax.scatter(list_min_leaf, list_max_nodes, list_accuracy_score)
...
#credit_model = np.load('TreeModel.npy',allow_pickle=True).item()
credit_model.fit(X_train, y_train)
...
np.save('TreeModel.npy',credit_model)
