...
def compute_bias(size_this, size_last):
    bias = np.zeros((size_this, 1)) + 0.1

    return bias

def compute_weights(size_this, size_last):
...
class MLP():
    ...
    def __init__(self, sizes):
        ...
        for l in range(self.num_layers):
            ...
            compute_weights(self.size[l+1], self.size[l])
            parameters["b" + str(l+1)] = \
            compute_bias(self.size[l+1], self.size[l])
    ...
    def train(self, training_data, epoches, batchsz, lr, test_data):
        ...
        for j in range(epoches):
            ...
            if test_data:
                ...
                print(self.evalueate(test_data))
            else:
...
if __name__ == '__main__':
    ...
    net = MLP([784, 30, 10])
    net.train(train_data, 10, 100, 0.1, test_data=test_data)
