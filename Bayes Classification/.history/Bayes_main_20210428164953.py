...
for i in y_test.index:
    ...
    for index in X_test.loc[i].keys().tolist():
        if index=='duration':
            rate=norm.pdf(list_new['duration'],loc=dict_gaussian['duration']['bad'][0],scale=dict_gaussian['duration']['bad'][1])
        elif index=='amount':
            rate=norm.pdf(list_new['amount'],loc=dict_gaussian['amount']['bad'][0],scale=dict_gaussian['amount']['bad'][1])
        elif index=='age':
            rate=norm.pdf(list_new['age'],loc=dict_gaussian['age']['bad'][0],scale=dict_gaussian['age']['bad'][1])
        else:
            rate=dict_main_false[index][list_new[index]]
        result_false=result_false*rate*10
